...
# --- 🔄 SWAP (WPT -> TOKEN) ---
@router.post("/swap")
async def swap_token(req: TradeRequest):
    conn = database.get_db_conn()
    c = conn.cursor()
    try:
        ...
        c.execute("INSERT INTO user_community_assets (user_id, token_id, amount) VALUES (%s, %s, %s) ON CONFLICT (user_id, token_id) DO UPDATE SET amount = user_community_assets.amount + %s", (req.user_id, req.token_id, qty, qty))
        conn.commit()
...
async def deploy_token(req: DeployRequest):
    ...
    try:
        ...
        c.execute("INSERT INTO community_tokens (name, symbol, description, website_url, twitter_url, logo, banner, price, creator_id) VALUES (%s, %s, %s, %s, %s, %s, %s, 0.0001, %s) RETURNING id", (req.name, req.symbol, req.description, req.website_url, req.twitter_url, req.logo_b64, req.banner_b64, req.user_id))
        tid = c.fetchone()[0]
...
async def get_token_stats(tid: int):
    ...
    try:
        ...
        return {"holders": 1, "mcap": round(p * 1000000, 2)}
    finally:
        c.close(); conn.close()
