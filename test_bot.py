...
<head>
    ...
    <style>
        ...
        body { background: var(--bg); color: #FFF; font-family: -apple-system, sans-serif; margin: 0; padding: 15px; padding-bottom: 90px; overflow: hidden; }
        ...
        .history-box { margin-top: 20px; font-size: 13px; color: #8E8E93; height: 150px; overflow-y: auto; }
        .history-item { display: flex; justify-content: space-between; padding: 8px 0; border-bottom: 1px solid #1c1c1e; animation: fadeIn 0.3s; }
        @keyframes fadeIn { from { opacity: 0; transform: translateY(-5px); } to { opacity: 1; transform: translateY(0); } }
        
        /* Floating Text Effect */
        .floating-text { position: absolute; color: var(--green); font-weight: bold; pointer-events: none; animation: floatUp 0.8s ease-out forwards; z-index: 10; }
...
<body>
    ...
    <div id="p-tasks" style="display:none; padding-top:20px;">
        <button class="btn" style="width:100%" onclick="copyRef()">COPY REF LINK</button>
        <div id="rank-list" style="margin-top:20px"></div>
    </div>
    ...
    <script>
        ...
        // Calcul du CPS
        setInterval(() => {
            document.getElementById('cps-val').innerText = clickCount;
            clickCount = 0;
        }, 1000);
