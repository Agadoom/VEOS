...
def patch_db():
    ...
    if conn:
        try:
            c = conn.cursor()
            # Ajout des colonnes nécessaires si elles manquent
            cols = [
...
async def get_user(uid: int):
    ...
    # Tentative de récupération du wallet séparément pour éviter l'Erreur 500
    wallet = ""
    try:
        c.execute("SELECT COALESCE(wallet_address, '') FROM users WHERE user_id=%s", (uid,))
        w_res = c.fetchone()
...
<head>
    ...
    <style>
        ...
        input { background: #111; border: 1px solid #333; color: #fff; padding: 12px; border-radius: 10px; width: 100%; box-sizing: border-box; margin: 10px 0; outline: none; }
    </style>
</head>
<body>
    ...
    <div id="p-wallet" style="display:none">
        ...
        <div class="card" style="flex-direction:column; align-items:flex-start;">
            ...
            <input type="text" id="w-addr" placeholder="Paste your TON address (UQC...)">
            <button class="btn" style="width:100%; background:var(--gold)" onclick="saveWallet()">SAVE ADDRESS</button>
        </div>
        <div class="card" style="margin-top:20px;">
            <div><b>Withdrawals</b><br><small>Available after TGE</small></div>
    ...
    <script>
        ...
        async function saveWallet() {
            ...
            if(addr.length < 10) return alert("Address too short");
        ...
        function show(p) {
            ['mine', 'pillars', 'leader', 'wallet', 'mission'].forEach(id => {
