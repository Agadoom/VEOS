...
def patch_db():
    ...
    if conn:
        try:
            c = conn.cursor()
            # Création d'une table séparée pour le wallet pour éviter de casser 'users'
            c.execute("CREATE TABLE IF NOT EXISTS user_wallets (user_id BIGINT PRIMARY KEY, address TEXT)")
...
async def get_user(uid: int):
    ...
    # On demande UNIQUEMENT les colonnes dont on est SUR qu'elles existent
    ...
    # On récupère le wallet dans la table séparée (Zéro risque de plantage)
    wallet = ""
    try:
        c.execute("SELECT address FROM user_wallets WHERE user_id=%s", (uid,))
        w_res = c.fetchone()
        if w_res: wallet = w_res[0]
    except: pass
...
async def wallet_api(request: Request):
    ...
    conn = get_db_conn(); c = conn.cursor()
    c.execute("INSERT INTO user_wallets (user_id, address) VALUES (%s, %s) ON CONFLICT (user_id) DO UPDATE SET address = EXCLUDED.address", (uid, addr))
...
<head>
    ...
    <style>
        ...
        .btn { background: #FFF; color: #000; border: none; padding: 10px 18px; border-radius: 12px; font-weight: 800; cursor: pointer; font-size: 11px; text-decoration: none; }
...
<body>
    <div id="p-mine">
        ...
        <div class="card"><div>VEO AI<br><small id="vv">0.00</small></div><button class="btn" onclick="mine('veo')">COMPUTE</button></div>
    </div>
    ...
    <div id="p-wallet" style="display:none">
        ...
        <input type="text" id="w-addr" placeholder="Paste TON address">
        <button class="btn" style="width:100%" onclick="saveWallet()">SAVE ADDRESS</button>
    </div>
    ...
    <script>
        ...
        async function saveWallet() { 
            ...
            await fetch('/api/wallet', {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({user_id:uid, address:addr})});
            alert("Saved!");
        }
        function show(p) {
            ['mine', 'pillars', 'wallet'].forEach(id => {
