...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("Your Node is Synchronized.", reply_markup=kb)
...
<head>
    ...
    <style>
        ...
        .status-led { height: 7px; width: 7px; background: var(--green); border-radius: 50%; display: inline-block; margin-right: 5px; box-shadow: 0 0 8px var(--green); animation: pulse 1.5s infinite; }
        ...
        .nav { position: fixed; bottom: 20px; left: 50%; transform: translateX(-50%); background: rgba(10,10,10,0.9); backdrop-filter: blur(20px); padding: 12px 25px; border-radius: 40px; display: flex; gap: 22px; border: 1px solid #333; z-index: 999; }
...
<body>
    ...
    <div class="machine-status">
        <span><span class="status-led"></span> NODE: OWPC-H1 ONLINE</span>
    ...
    <div id="p-mine">
        ...
        <div class="card"><div><small style="color:var(--green)">GENESIS</small><div id="gv" style="font-weight:700">0.00</div></div><button class="btn m-btn" onclick="mine('genesis')">MINE</button></div>
    ...
    <div id="p-mission" style="display:none">
        ...
        <div style="background:#111; padding:15px; border-radius:15px; border:1px solid #222; font-size:12px;">
            <p>1. Alpha Hub Launch (Done)</p>
            <p>2. DePIN Hardware Sync (2026)</p>
            <p>3. Global Exchange Listing</p>
        </div>
    </div>
