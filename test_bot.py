...
async def get_user(uid: int):
    ...
    return {
        ...
        "machine_load": random.randint(88, 99), "multiplier": round(1.0 + ((r[9] or 0) / 100) * 0.1 + (score / 1000), 2),
...
<body>
    ...
    <div id="p-mine">
        ...
        <div class="card"><div><small>GENESIS</small><div id="gv">0.00</div></div><button class="btn" onclick="mine(event, 'genesis')">MINE</button></div>
        <div class="card"><div><small>UNITY</small><div id="uv">0.00</div></div><button class="btn" onclick="mine(event, 'unity')">SYNC</button></div>
        <div class="card"><div><small>VEO AI</small><div id="vv">0.00</div></div><button class="btn" onclick="mine(event, 'veo')" style="background:#A259FF; color:#FFF">COMPUTE</button></div>
    </div>

    <div id="p-mission" style="display:none; padding-top:20px;">
        ...
        <button class="btn" style="width:100%; background:var(--blue); color:#FFF; padding:15px;" onclick="tg.openTelegramLink('https://t.me/share/url?url=https://t.me/owpcsbot?start='+uid)">🚀 INVITE FRIENDS</button>
    </div>
    ...
    <script>
        ...
        function checkDailyStatus() {
            const lastClaim = localStorage.getItem('last_claim_' + uid);
            const btn = document.getElementById('daily-btn');
            if (lastClaim) {
                const diff = Date.now() - parseInt(lastClaim);
                if (diff < TWELVE_HOURS) {
                    const remaining = Math.ceil((TWELVE_HOURS - diff) / (60 * 60 * 1000));
                    btn.innerText = "⏳ " + remaining + "H";
                    btn.disabled = true;
        ...
        async function claimDaily() {
            ...
            try {
                ...
                if(res.ok) {
                    localStorage.setItem('last_claim_' + uid, Date.now().toString());
        ...
        function show(p) { 
            ...
            document.getElementById('p-mission').style.display = (p==='mission'?'block':'none');
            document.getElementById('n-mine').classList.toggle('active', p==='mine');
            document.getElementById('n-mission').classList.toggle('active', p==='mission');
        }

        refresh();
        setInterval(checkDailyStatus, 60000); // Vérifie chaque minute
    </script>
</body>
</html>
"""
