...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("Connected to OWPC DePIN Network.", reply_markup=kb)
...
async def get_user(uid: int):
    ...
    now = int(time.time())
    mins = (now - r[7]) // 60
    current_energy = min(MAX_ENERGY, r[6] + (mins * REGEN_RATE))
    ...
    return {
        ...
        "energy": current_energy, "max_energy": MAX_ENERGY, "badge": get_badge(r[0]+r[1]+r[2], r[8]),
        "top": top, "feed": feed, "jackpot": round(total_mined * 0.1, 2),
        "machine_load": random.randint(85, 98), "price_wpt": 0.00045
    }
...
async def mine_api(request: Request):
    ...
    if current_e >= 1:
        c.execute(f"UPDATE users SET p_{t} = p_{t} + 0.05, energy = %s, last_energy_update = %s WHERE user_id = %s", 
                  (current_e - 1, now, uid))
...
<body>
    <div class="machine-status"><span>● NODE: OWPC-H1 ONLINE</span><span>LOAD: <span id="m-load">0</span>%</span></div>
    
    <div class="profile-bar">
        <div style="display:flex; align-items:center; gap:10px;"><div id="u-name" style="font-weight:700">...</div><div id="u-badge" class="badge-tag">Bronze</div></div>
        <div id="u-ref" style="font-weight:bold; font-size:12px; color:var(--gold)">0 REFS</div>
    </div>

    <div id="p-mine">
        <div class="balance">
            <small style="color:var(--text)">NETWORK ASSETS</small>
        ...
        <div class="card"><div><small style="color:var(--green)">GENESIS</small><div id="gv" style="font-weight:700">0.00</div></div><button class="btn mine-btn" onclick="mine('genesis')">MINE</button></div>
        <div class="card"><div><small style="color:var(--blue)">UNITY</small><div id="uv" style="font-weight:700">0.00</div></div><button class="btn mine-btn" onclick="mine('unity')">SYNC</button></div>
        <div class="card"><div><small style="color:#A259FF">VEO AI</small><div id="vv" style="font-weight:700">0.00</div></div><button class="btn mine-btn" onclick="mine('veo')" style="background:#A259FF; color:#FFF">COMPUTE</button></div>
    </div>

    <div id="p-mission" style="display:none">
        <h3 style="color:var(--gold)">ROADMAP</h3>
        <div class="card" onclick="window.open('https://t.me/OWPC_Co', '_blank')"><b>Join Community</b><span>💬</span></div>
        <div style="padding:15px; background:#111; border-radius:15px; border:1px solid #222; font-size:12px;">
            <p>• Phase 1: Alpha Mining (Current)</p>
            <p>• Phase 2: DePIN Machine Integration</p>
            <p>• Phase 3: Mainnet Listing</p>
        </div>
    </div>
    ...
    <script>
        ...
        async function refresh() {
            ...
            let perc = (d.energy / d.max_energy * 100);
            document.getElementById('e-bar').style.width = perc + "%";
            document.getElementById('e-text').innerText = `⚡ ${d.energy} / ${d.max_energy}`;
            
            // Griser les boutons si plus d'énergie
            document.querySelectorAll('.mine-btn').forEach(b => b.disabled = (d.energy < 1));
