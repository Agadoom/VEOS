    ...
    <div class="container">
        ...
        <div id="p-launcher" class="page">
            ...
            <div class="card" style="flex-direction:column; align-items:stretch; gap:12px; background:linear-gradient(to bottom, #121214, #000);">
                ...
                <button class="btn" style="background:var(--gold); padding:15px; font-size:14px;" onclick="deployStars()">LAUNCH FOR 500 ★</button>
            </div>

            <h3 style="margin:20px 0 10px 5px;">🔥 LIVE MARKET</h3>
    ...
    <script>
        ...
        async function deployStars() {
            ...
            const save = await fetch('/api/launcher/save-pending', {
                ...
                body: JSON.stringify({user_id:uid, name:name, symbol:sym, desc:document.getElementById('tk-desc').value, logo:b64_logo, banner:b64_banner})
            });
            ...
            tokens.forEach(t => {
                h += `<div class="card" onclick='openToken(${JSON.stringify(t)})'>
                    <div style="display:flex; align-items:center; gap:12px;">
                        <img src="${t.logo}" style="width:40px;height:40px;border-radius:10px;object-fit:cover;">
