...
async def api_get_user(uid: int):
    ...
    prices = {"gold": 2150.40 + random.uniform(-2, 2), "silver": 24.15 + random.uniform(-0.2, 0.2), "copper": 3.85}
    ...
    return {
        ...
        "news": "🔥 FRENZY MODE" if is_frenzy else "🟢 Market Active", "prices": prices,
...
async def api_mine(request: Request):
    ...
    conn = database.get_db_conn(); c = conn.cursor()
    # FIX: On récupère l'énergie actuelle pour valider le clic
    ...
    if res:
        # Anti-spam : 80ms minimum entre clics
...
# --- WEB UI (5 SECTIONS + FRENZY + PROFILE) ---
...
<head>
    ...
    <style>
        ...
        .ticker-wrap { display: inline-block; animation: t 25s linear infinite; }
        ...
        /* Frenzy Visual */
        .f-glow { position: fixed; inset: 0; border: 4px solid var(--red); pointer-events: none; z-index: 99; display: none; animation: p 1s infinite; }
        @keyframes p { 0%, 100% { opacity: 0.1; } 50% { opacity: 0.4; } }
        ...
        .xp-f { background: var(--purple); height: 100%; border-radius: 3px; width: 0%; transition: 1s; }
...
<body>
    ...
    <div class="ticker"><div class="ticker-wrap">
    ...
    <div id="p-profile" style="display:none">
        <div style="text-align:center; padding:20px;">
            ...
            <h2 id="pr-n">...</h2>
            <div id="pr-b" style="color:var(--gold); font-size:12px; font-weight:bold;">...</div>
            <div class="xp-b"><div id="xp-f" class="xp-f"></div></div>
            <small id="xp-t" style="color:var(--text); font-size:9px;">Next Rank: ...</small>
        </div>
        <div class="card"><span>Network Power</span><b id="pr-m">x1.0</b></div>
        <div class="card"><span>Streak</span><b id="pr-s">0 Days</b></div>
        <div class="card"><span>Staked WPT</span><b id="pr-st">0</b></div>
    </div>
    ...
    <div id="p-opps" style="display:none">
        ...
        <div class="card" style="display:grid; grid-template-columns:1fr 1fr 1fr; gap:5px; text-align:center;">
            <div><small style="color:var(--gold)">GOLD</small><br><b id="pg" style="font-size:10px;">$---</b></div>
            <div><small style="color:var(--text)">SILVER</small><br><b id="ps" style="font-size:10px;">$---</b></div>
            <div><small style="color:var(--purple)">COPPER</small><br><b id="pc" style="font-size:10px;">$---</b></div>
        </div>
        <div class="card" style="border:1px solid var(--purple); flex-direction:column; margin-top:10px;">
            ...
            <div style="display:grid; grid-template-columns:1fr 1fr; gap:10px; width:100%; margin-top:10px;">
                <div style="background:#000; padding:10px; border-radius:10px; text-align:center;"><small style="font-size:8px;">MY REFS</small><br><b id="o-r" style="color:var(--purple)">0</b></div>
                <div style="background:#000; padding:10px; border-radius:10px; text-align:center;"><small style="font-size:8px;">EST. EARN</small><br><b id="o-e" style="color:var(--green)">0.00</b></div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        async function refresh() {
            try {
                ...
                document.getElementById('pg').innerText = "$" + d.prices.gold.toFixed(2);
                document.getElementById('ps').innerText = "$" + d.prices.silver.toFixed(2);
                document.getElementById('pc').innerText = "$" + d.prices.copper.toFixed(2);
        ...
        tg.expand(); refresh(); setInterval(refresh, 5000); // Refresh toutes les 5s pour plus de réactivité
    </script>
</body>
</html>
import asyncio, uvicorn, time, random
