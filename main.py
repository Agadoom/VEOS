...
async def api_get_user(uid: int):
    ...
    return {
        ...
        "energy": en, "max_energy": config.MAX_ENERGY, "score": round(score, 2), "badge": badge, "badge_color": color,
...
async def api_deploy_token(request: Request):
    ...
    try:
        ...
        res = c.fetchone()
        if not res or float(res[0] or 0) < 500: return JSONResponse(status_code=400, content={"error": "Need 500 Genesis"})
