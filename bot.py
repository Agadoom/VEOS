...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.args and context.args[0].startswith("ref_"):
        ...
        if not c.fetchone() and user.id != ref_id:
            ...
            conn.commit()
            try:
                await context.bot.send_message(ref_id, f"🔥 +50 PTS! {user.first_name} joined via your link!")
            except: pass
        conn.close()

    # --- Réponse pour chat privé ---
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # --- Comptage points dans le groupe ---
    ...
    if is_private or chat.id == GROUP_CHAT_ID or is_mentioned or is_reply:
        try:
            res = openai.ChatCompletion.create(
                model="gpt-3.5-turbo",
                messages=[
                    {"role": "system", "content": "Tu es l'IA Alpha OWPC. Pro et visionnaire."},
